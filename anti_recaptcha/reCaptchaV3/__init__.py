...
from anti_recaptcha.utils import parse_url, random_delay
...
class reCaptchaV3:
	...
	def __new__(cls, *args, **kwargs) -> str:
		...
		token = cls.get_recaptcha_token(data['endpoint'], data['params'])
		...
		recaptcha_response = cls.get_recaptcha_response(data['endpoint'], f'k={params["k"]}', post_data)
		
		return recaptcha_response
	
	def __init__(self, anchor_url: str, proxy: Union[Proxy, Dict] = None, timeout: Union[int, float] = 20):
	...
	def get_recaptcha_token(endpoint: str, params: str) -> str:
		...
		"""
		random_delay()
		response = reCaptchaV3.session.send_request(f"{endpoint}/anchor", params=params)
	...
	def get_recaptcha_response(endpoint: str, params: str, data: str) -> str:
		...
		"""
		random_delay()
		response = reCaptchaV3.session.send_request(f"{endpoint}/reload", data=data, params=params)
